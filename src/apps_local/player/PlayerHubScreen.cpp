...
void buildPlayerHub(toybox::Screen& screen, const Model& model) {
  ...
  // Keep the player actions fixed in place so changing guest eligibility does
  // not cause an expensive full-layout jump on the e-ink panel.
  const fui::Rect actions = screen.takeBottom(toybox::kRowHeight, toybox::kGutter);
  constexpr int kActionGap = 6;
  const int16_t actionWidth = static_cast<int16_t>((actions.width - 3 * kActionGap) / 4);
  ...
  const fui::Rect profileButton =
      fui::makeRect(static_cast<int16_t>(callsignButton.right() + kActionGap), actions.y, actionWidth, actions.height);
  const fui::Rect registerButton =
      fui::makeRect(static_cast<int16_t>(profileButton.right() + kActionGap), actions.y,
                    static_cast<int16_t>(actions.right() - profileButton.right() - kActionGap), actions.height);
