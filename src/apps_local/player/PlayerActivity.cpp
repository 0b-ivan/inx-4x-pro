...
bool PlayerActivity::refreshProfile() {
  profileStatsCount_ = 0;
  if (!player::runtime().ready()) {
    setMessage("PLAYER STORAGE UNAVAILABLE");
    return false;
  }

  const player::StoreResult result =
      player::runtime().currentStats(profileStats_.data(), profileStats_.size(), profileStatsCount_);
  if (result != player::StoreResult::Ok) {
    setMessage("PROFILE STATS FAILED");
    return false;
  }
  return true;
}
...
void PlayerActivity::loop() {
  ...
  if (event.action == playerhubui::ActionViewProfile) {
    if (refreshProfile()) {
      view_ = View::Profile;
      setMessage("");
      requestUpdate();
...
void PlayerActivity::render(RenderLock&&) {
  ...
  if (view_ == View::Profile && player::runtime().ready()) {
    char callsign[player::kMaxNameLength + 1]{};
    const player::Player* active = player::runtime().activePlayer();
    const bool guest = active == nullptr;
    const char* currentName = guest ? "GUEST" : active->name;
    if (guest && player::runtime().guest().active()) {
    ...
    for (size_t i = 0; i < profileStatsCount_; ++i) {
      if (profileStats_[i].game == player::GameId::Battleship) {
        battleship = profileStats_[i];
        break;
      }
    }

    const player::ProgressionSnapshot progression =
        player::ProgressionSystem::summarize(profileStats_.data(), profileStatsCount_);
    const leaderboard::Rank rank = leaderboard::RankSystem::forWins(battleship.wins);

    playerprofileui::Model model;
    model.name = currentName;
    model.callsign = callsign;
    model.guest = guest;
    model.progression = progression;
    model.battleship = battleship;
    model.battleshipRank = rank.name;
    model.nextBattleshipRankWins = leaderboard::RankSystem::nextThreshold(battleship.wins);
    playerprofileui::buildPlayerProfile(screen, model, renderer);
  } else if (view_ == View::Callsign && player::runtime().ready() && player::runtime().guest().active()) {
