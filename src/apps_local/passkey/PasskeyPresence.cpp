...
bool PasskeyPresence::consumeApproval() {
  uint8_t expected = static_cast<uint8_t>(PresenceDecision::Approved);
  if (!decision_.compare_exchange_strong(expected, static_cast<uint8_t>(PresenceDecision::Idle),
                                         std::memory_order_acq_rel)) {
    return false;
  }
