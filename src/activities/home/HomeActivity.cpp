...
void HomeActivity::loop() {
  ...
  auto activateSelection = [this] {
    ...
    // Shelf folders are appended after the four upstream rows (and optional
    // Continue Reading row). Dispatch them before the upstream helper so a
    // theme/recent-book offset can never route Games into Apps or vice versa.
    const int shelfBase = upstreamMenuRows();
    if (menuIndex >= shelfBase) {
      const int shelfRow = menuIndex - shelfBase;
