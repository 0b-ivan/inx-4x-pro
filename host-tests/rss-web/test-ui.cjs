...
(async () => {
  ...
  try {
    ...
    const posts=[];
    let reading={newestFirst:true,showRead:false};
    ...
    await page.route('http://reader.test/**', async route=>{
      ...
      if (url.pathname === '/api/rss/reading') {
        if (req.method() === 'POST') reading=req.postDataJSON();
    ...
    await page.locator('#rss-newest-first').uncheck();
    await page.locator('#rss-show-read').check();
    await page.getByRole('button',{name:'Save reading preferences',exact:true}).click();
    await page.waitForFunction(()=>document.getElementById('message').textContent.startsWith('Reading preferences saved'));
    await page.reload();
    await page.waitForFunction(()=>document.getElementById('rss-show-read')?.checked);
    assert.equal(await page.locator('#rss-newest-first').isChecked(),false);
