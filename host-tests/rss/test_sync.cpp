...
std::set<std::string> pending, failures, readItems;
...
namespace rsscache {
bool isRead(const RssFeed&, const RssItem& item) { return readItems.count(item.guid); }
bool sameItem(const RssItem& a, const RssItem& b) { return a.guid == b.guid; }
bool loadFeed(const RssFeed& feed, std::string&, std::vector<RssItem>& items, RssCacheInfo*, bool includeRead) {
  items = indexes[feed.url];
  if (!includeRead) std::erase_if(items, [&](const RssItem& item) { return isRead(feed, item); });
  return !items.empty();
}
